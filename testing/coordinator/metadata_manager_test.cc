...
TEST_P(EntryOperationTest, TestEntryOperations) {
  ...
  if (test_case.expect_num_broadcasts > 0) {
    // Add missing cluster mock expectations (copied from TestBroadcastMetadata)
...
TEST_P(MetadataManagerReconciliationTest, TestReconciliation) {
  ...
  if (test_case.expect_broadcast) {
    // Add cluster mock expectations for broadcast functionality
  ...
  // Expect FT.INTERNAL_UPDATE calls instead of direct callbacks
  if (!test_case.expected_callbacks.empty()) {
    // Check if any registered type expects to return an error
    ...
    if (expect_failure) {
      // Mock FT.INTERNAL_UPDATE to return nullptr (failure)
      EXPECT_CALL(*kMockValkeyModule,
...
TEST_F(MetadataManagerTest, TestBroadcastMetadata) {
  ...
  // Mock the cluster operations for broadcast
  EXPECT_CALL(*kMockValkeyModule, GetClusterNodesList(fake_ctx, testing::_))
      .WillOnce([](ValkeyModuleCtx *ctx, size_t *numnodes) {
        *numnodes = 1;
        char **ids = new char *[1];
...
TEST_F(MetadataManagerTimestampTest,
  ...
  // Mock FT.INTERNAL_UPDATE to fail to simulate reconciliation failure
  EXPECT_CALL(*kMockValkeyModule,
              Call(testing::_, testing::StrEq("FT.INTERNAL_UPDATE"), testing::StrEq("!Kcbb"),
                   testing::_, testing::_, testing::_, testing::_, testing::_))
